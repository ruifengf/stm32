...
_Bool ESP8266_Sys_WaitRecive(void)
{
	...
	if(esp8266_sys_cnt == esp8266_sys_cntPre)				//如果上一次的值和这次相同，则说明接收完毕
	{
		...
		return SYS_REV_OK;								//返回接收完成标志
	}
		
	esp8266_sys_cntPre = esp8266_sys_cnt;					//置为相同
	
	return SYS_REV_WAIT;								//返回接收未完成标志

}

//==========================================================
//	函数名称：	ESP8266_Sys_SendCmd
//
//	函数功能：	发送命令
//
//	入口参数：	cmd：命令
...
{	
	...
	while(time--)
	{
		if(ESP8266_Sys_WaitRecive() == SYS_REV_OK)							//如果收到数据
		{
			printf("test\r\n");
			if(strstr((const char *)esp8266_sys_buf, res) != NULL)		//如果检索到关键词
			{
				ESP8266_Sys_Clear();									//清空缓存
...
void USART3_IRQHandler(void)
{
	...
	{
		if(esp8266_sys_cnt >= sizeof(esp8266_sys_buf))	esp8266_sys_cnt = 0; //防止串口被刷爆
